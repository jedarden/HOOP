
// ===== agent_config.json =====

/**
 * Human-interface agent configuration
 */
export interface AgentConfig {
  /**
   * Adapter selection
   */
  adapter?: "claude" | "codex" | "opencode" | "gemini" | "zai";
  /**
   * Model name (e.g., opus, sonnet, haiku)
   */
  model?: string;
  /**
   * Optional rate limit
   */
  rate_limit_requests_per_minute?: number;
  /**
   * Optional cost cap per session
   */
  cost_cap_per_session_usd?: number;
  /**
   * Idle timeout: no events, any type, for N seconds (default: 180)
   */
  idle_timeout_secs?: number;
  /**
   * Hard ceiling on total turn duration regardless of activity (default: 3600)
   */
  max_runtime_secs?: number;
  /**
   * Extended idle tolerance once real content appeared (default: 600)
   */
  content_seen_grace_secs?: number;
  /**
   * No heartbeat state transition (Live<->Hung) for N seconds (default: 300 = 5 minutes)
   */
  heartbeat_transition_threshold_secs?: number;
  /**
   * Maximum retry attempts on the same bead before alerting (default: 3)
   */
  retry_threshold?: number;
}


// ===== audit_row.json =====

/**
 * Audit log entry for every write HOOP performs
 */
export interface AuditRow {
  /**
   * Unique audit entry ID
   */
  id: string;
  /**
   * Timestamp of the action
   */
  ts: string;
  /**
   * Actor identity (e.g., 'user:jedarden', 'hoop:agent:session-id')
   */
  actor: string;
  /**
   * Action kind
   */
  kind:
    | "bead_created"
    | "stitch_created"
    | "config_changed"
    | "project_added"
    | "project_removed"
    | "schema_migrated"
    | "backup_started"
    | "backup_finished"
    | "backup_failed"
    | "restore_started"
    | "restore_finished"
    | "restore_failed";
  /**
   * Target identifier (bead ID, stitch ID, etc.)
   */
  target: string;
  /**
   * Arguments passed to the action
   */
  args?: {
    [k: string]: unknown;
  };
  /**
   * Action result
   */
  result: "success" | "failure" | "partial";
  /**
   * Error message if result is not success
   */
  error?: string | null;
  /**
   * SHA-256 hash of the previous audit row (64 hex chars, genesis row is all zeros)
   */
  hash_prev: string;
  /**
   * SHA-256 hash of this audit row's content (64 hex chars)
   */
  hash_self: string;
  /**
   * Schema version at time of action
   */
  schema_version?: string;
  [k: string]: unknown;
}


// ===== backup_config.json =====

/**
 * S3-compatible backup configuration. Credentials are resolved from environment variables only — never stored in this file.
 */
export interface BackupConfig {
  /**
   * S3-compatible endpoint URL
   */
  endpoint: string;
  /**
   * Bucket name
   */
  bucket: string;
  /**
   * Key prefix for backup objects
   */
  prefix: string;
  /**
   * Cron schedule for automatic backups (default: daily at 04:00 UTC)
   */
  schedule?: string;
  /**
   * Number of days to retain backups before pruning
   */
  retention_days?: number;
  /**
   * Encrypt backups with age (requires HOOP_BACKUP_AGE_KEY env var)
   */
  encryption?: boolean;
}


// ===== bead.json =====

/**
 * A tracking bead (issue/task) in the beads system
 */
export interface Bead {
  /**
   * Bead ID (e.g., hoop-ttb.1)
   */
  id: string;
  /**
   * Bead title
   */
  title: string;
  /**
   * Bead description
   */
  description?: string | null;
  /**
   * Bead status
   */
  status: "open" | "closed";
  /**
   * Priority (0 = highest)
   */
  priority: number;
  /**
   * Issue type
   */
  issue_type: "task" | "bug" | "epic" | "genesis" | "review" | "fix";
  /**
   * Creation timestamp
   */
  created_at: string;
  /**
   * Last update timestamp
   */
  updated_at: string;
  /**
   * Creator username
   */
  created_by: string;
  /**
   * List of dependent bead IDs
   */
  dependencies: string[];
  schema_version: string;
  [k: string]: unknown;
}


// ===== bead_created_by_hoop.json =====

/**
 * Event broadcast after each successful br create via HOOP
 */
export interface BeadCreatedByHoop {
  /**
   * Project name where the bead was created
   */
  project: string;
  /**
   * ID of the created bead
   */
  bead_id: string;
  /**
   * Identity that created the bead (e.g., 'tailscale:user@example.com' or 'os:username')
   */
  actor: string;
  /**
   * Source of the bead creation (e.g., 'form', 'chat', 'bulk', 'template:<name>')
   */
  source: string;
  /**
   * ISO 8601 timestamp of when the bead was created
   */
  ts: string;
  [k: string]: unknown;
}


// ===== bead_data.json =====

/**
 * Bead data sent to the frontend
 */
export interface BeadData {
  /**
   * Bead ID (e.g., hoop-ttb.1)
   */
  id: string;
  /**
   * Bead title
   */
  title: string;
  /**
   * Bead status
   */
  status: "open" | "closed";
  /**
   * Priority (0 = highest)
   */
  priority: number;
  /**
   * Issue type
   */
  issue_type: "task" | "bug" | "epic";
  /**
   * ISO 8601 timestamp
   */
  created_at: string;
  /**
   * ISO 8601 timestamp
   */
  updated_at: string;
  /**
   * Creator username
   */
  created_by: string;
  /**
   * List of dependent bead IDs
   */
  dependencies: string[];
  [k: string]: unknown;
}


// ===== capacity_limits.json =====

/**
 * Capacity limits for an adapter account
 */
export interface CapacityLimits {
  /**
   * Token limit per 5-hour window (Claude-specific)
   */
  tokens_per_5h?: number | null;
  /**
   * Token limit per 7-day window (Claude-specific)
   */
  tokens_per_7d?: number | null;
  /**
   * Prompt limit per 5-hour window (OpenCode ZAI proxy)
   */
  prompts_per_5h?: number | null;
  /**
   * Prompt limit per 7-day window (OpenCode ZAI proxy)
   */
  prompts_per_7d?: number | null;
  /**
   * Request limit per day
   */
  requests_per_day?: number | null;
  /**
   * Spend limit per day in USD
   */
  spend_usd_per_day?: number | null;
  /**
   * Concurrent request limit
   */
  concurrent_requests?: number | null;
  [k: string]: unknown;
}


// ===== capacity_usage.json =====

/**
 * Current capacity usage for an adapter account
 */
export interface CapacityUsage {
  /**
   * Tokens used in current 5-hour window
   */
  tokens_5h: number;
  /**
   * Tokens used in current 7-day window
   */
  tokens_7d: number;
  /**
   * Prompts used in current 5-hour window (OpenCode ZAI proxy)
   */
  prompts_5h: number;
  /**
   * Prompts used in current 7-day window (OpenCode ZAI proxy)
   */
  prompts_7d: number;
  /**
   * Requests made today
   */
  requests_today: number;
  /**
   * USD spent today
   */
  spend_usd_today: number;
  /**
   * Currently active requests
   */
  active_requests?: number;
  [k: string]: unknown;
}


// ===== codex_account_daily_spend_row.json =====

/**
 * Per-account daily spend row from codex_account_daily_spend (§6 Phase 2 §10)
 */
export interface CodexAccountDailySpendRow {
  /**
   * Codex account identifier (e.g. 'default', 'work', 'personal')
   */
  account_id: string;
  /**
   * Date in YYYY-MM-DD format
   */
  date: string;
  /**
   * Plan tier name (e.g. 'tier_1', 'tier_2', 'free')
   */
  plan_tier: string;
  /**
   * Total spend in USD for this account on this date
   */
  cost_usd: number;
  /**
   * Total input tokens consumed
   */
  input_tokens: number;
  /**
   * Total output tokens generated
   */
  output_tokens: number;
  /**
   * RFC-3339 timestamp of last snapshot write
   */
  updated_at: string;
  [k: string]: unknown;
}


// ===== codex_account_monthly_rollup_row.json =====

/**
 * Per-account monthly spend rollup aggregated from codex_account_daily_spend (§6 Phase 2 §10)
 */
export interface CodexAccountMonthlyRollupRow {
  /**
   * Codex account identifier (e.g. 'default', 'work', 'personal')
   */
  account_id: string;
  /**
   * Month in YYYY-MM format
   */
  month: string;
  /**
   * Most recent plan tier seen for this account in this month
   */
  plan_tier: string;
  /**
   * Total spend in USD for this account in this month
   */
  cost_usd: number;
  /**
   * Total input tokens consumed in this month
   */
  input_tokens: number;
  /**
   * Total output tokens generated in this month
   */
  output_tokens: number;
  [k: string]: unknown;
}


// ===== config_error.json =====

/**
 * Configuration parse error for projects.yaml hot-reload
 */
export interface ConfigError {
  /**
   * Human-readable error message
   */
  message: string;
  /**
   * Line number where the error occurred (1-indexed)
   */
  line: number;
  /**
   * Column number where the error occurred (1-indexed)
   */
  col: number;
  [k: string]: unknown;
}


// ===== cross_workspace_blocker.json =====

/**
 * A cross-workspace blocker entry for a bead. Represents a bead in another workspace that blocks completion via Stitch-child relationships (§4.2).
 */
export interface CrossWorkspaceBlocker {
  /**
   * Bead ID that is blocking completion
   */
  bead_id: string;
  /**
   * Workspace where the bead exists (for cross-workspace context)
   */
  workspace: string;
  /**
   * Bead title
   */
  title: string;
  /**
   * Bead status (e.g., 'Open', 'Closed')
   */
  status: string;
  /**
   * Bead priority (1-5, where 1 is highest)
   */
  priority: number;
  /**
   * Bead issue type (e.g., 'task', 'bug', 'feature')
   */
  issue_type: string;
  [k: string]: unknown;
}


// ===== debug_state.json =====

/**
 * Complete daemon state snapshot for incident triage (§16.8, §M3). Field additions require a schema bump (§20).
 */
export interface DebugState {
  /**
   * SemVer version of this payload schema. Bump on any field addition or removal (§20).
   */
  schema_version: string;
  /**
   * Seconds elapsed since the daemon process started.
   */
  uptime_secs: number;
  /**
   * Version identifiers for the running daemon and schema crate.
   */
  version: {
    /**
     * hoop-daemon Cargo package version (semver).
     */
    daemon: string;
    /**
     * hoop-schema SCHEMA_VERSION constant.
     */
    schema: string;
  };
  /**
   * SHA-256 hex digest of the serialised resolved config (§17.2). Changes when any config key changes source or value.
   */
  config_hash: string;
  /**
   * Socket address the HTTP server is bound to.
   */
  bind_addr: string;
  /**
   * All workers currently known to the registry with full liveness detail.
   */
  workers: {
    name: string;
    /**
     * Formatted WorkerDisplayState (e.g. 'Executing { bead: … }').
     */
    state: string;
    liveness: "Live" | "Hung" | "Dead";
    last_heartbeat: string;
    heartbeat_age_secs: number;
    /**
     * Last observed OS PID for this worker, null when not in Executing state.
     */
    pid?: number | null;
  }[];
  /**
   * Every worker PID observed (latest PID per worker name). Useful for kill -0 checks.
   */
  worker_pids: {
    worker: string;
    pid: number;
  }[];
  /**
   * Every currently active claim: workers in Executing state with the bead they hold.
   */
  active_claims: {
    worker: string;
    bead: string;
    adapter: string;
    pid?: number | null;
  }[];
  /**
   * Every currently connected WebSocket client with its connection ID and age.
   */
  ws_clients: {
    conn_id: number;
    connected_at: string;
    connected_secs: number;
  }[];
  /**
   * Mapping of CLI session IDs to their worker/bead aliases (dual-identity invariant §3.4).
   */
  session_alias_table: {
    session_id: string;
    provider: string;
    worker?: string | null;
    bead?: string | null;
    strand?: string | null;
  }[];
  /**
   * Backup health indicators (§15, §16.6).
   */
  backup_timestamps: {
    /**
     * Unix timestamp (seconds) of last successful backup, 0 if never.
     */
    last_success_unix: number;
    /**
     * ISO 8601 of last successful backup, null if never.
     */
    last_success_iso: string | null;
    /**
     * Size of last backup in bytes, 0 if never.
     */
    last_size_bytes: number;
  };
  fleet_db_path: string;
  fleet_db_size_bytes: number;
  fleet_db_wal_size_bytes: number;
  open_stitches: number;
  total_beads: number;
  /**
   * Names of all projects currently known to the supervisor.
   */
  projects: string[];
}


// ===== dictated_note.json =====

/**
 * Metadata for a dictated note stitch — audio + transcript stored as a first-class project entity
 */
export interface DictatedNote {
  /**
   * FK to the parent stitch (kind=dictated)
   */
  stitch_id: string;
  /**
   * When the audio was captured
   */
  recorded_at: string;
  /**
   * When Whisper transcription completed
   */
  transcribed_at: string;
  /**
   * Filename of the audio attachment in ~/.hoop/attachments/<stitch_id>/
   */
  audio_filename: string;
  /**
   * Full text transcript of the audio
   */
  transcript: string;
  /**
   * Word-level timestamps from Whisper for transcript sync
   */
  transcript_words?: {
    /**
     * The word text
     */
    word: string;
    /**
     * Start time in seconds
     */
    start: number;
    /**
     * End time in seconds
     */
    end: number;
    [k: string]: unknown;
  }[];
  /**
   * Audio duration in seconds
   */
  duration_secs?: number | null;
  /**
   * Detected language code (e.g. en, de)
   */
  language?: string | null;
  /**
   * Optional tags for categorization
   */
  tags?: string[];
  /**
   * Whether transcription is pending, completed, or failed
   */
  transcription_status?: "Pending" | "Completed" | "Failed";
  /**
   * Words that have been redacted from the transcript (with timestamps for audio muting)
   */
  redacted_words?: {
    /**
     * Index into the original transcript_words array
     */
    word_index: number;
    /**
     * The original word that was redacted (for audit trail)
     */
    original_word: string;
    /**
     * Start timestamp of the word (for audio muting)
     */
    start: number;
    /**
     * End timestamp of the word (for audio muting)
     */
    end: number;
    /**
     * When the redaction was performed
     */
    redacted_at: string;
    [k: string]: unknown;
  }[];
  schema_version: string;
  [k: string]: unknown;
}


// ===== embedding_config.json =====

/**
 * Embedding service configuration for semantic deduplication and similarity search (§6.10.1). Supports local transformer models, remote APIs, and caching.
 */
export interface EmbeddingConfig {
  /**
   * Embedding adapter to use: 'local' (BGE-small transformer), 'remote' (Anthropic API), or 'cached' (local with caching). Hot-reloadable.
   */
  adapter?: "local" | "remote" | "cached";
  /**
   * Enable in-memory caching of embeddings keyed by content hash. Hot-reloadable.
   */
  cache_enabled?: boolean;
  /**
   * Cache entry TTL in seconds (default: 24 hours). Expired entries are evicted on next access. Hot-reloadable.
   */
  cache_ttl_seconds?: number;
  /**
   * Rate limit for remote embedding API calls in requests per minute. Null = no limit. Only applies to 'remote' adapter. Hot-reloadable.
   */
  rate_limit_rpm?: number | null;
}


// ===== message_usage.json =====

/**
 * Token usage from a single message
 */
export interface MessageUsage {
  /**
   * Input tokens (prompt)
   */
  input_tokens: number;
  /**
   * Output tokens (completion)
   */
  output_tokens: number;
  /**
   * Cache read tokens (prompt cache hits)
   */
  cache_read_tokens: number;
  /**
   * Cache write tokens (new cache entries)
   */
  cache_write_tokens: number;
  [k: string]: unknown;
}


// ===== morning_brief_config.json =====

/**
 * Morning Brief configuration (marquee #10, Phase 5). Autonomous daily briefing generator that reviews overnight activity and pre-drafts follow-up Stitches.
 */
export interface MorningBriefConfig {
  /**
   * How many hours back to look for overnight activity (default: 24, max: 168/7 days)
   */
  window_hours?: number;
  /**
   * Hour of day (0-23) to auto-run the brief in local time (default: 7 = 7 AM)
   */
  schedule_hour?: number;
  /**
   * Whether the scheduled auto-run is enabled (default: true)
   */
  auto_run_enabled?: boolean;
}


// ===== pattern.json =====

/**
 * A grouping of stitches toward a goal
 */
export interface Pattern {
  /**
   * Unique pattern identifier
   */
  id: string;
  /**
   * Pattern title
   */
  title: string;
  /**
   * Pattern description
   */
  description?: string | null;
  /**
   * Pattern status
   */
  status: "planned" | "active" | "blocked" | "done" | "abandoned";
  /**
   * Pattern owner
   */
  owner?: string | null;
  /**
   * Optional deadline
   */
  deadline?: string | null;
  /**
   * Parent pattern ID for nesting
   */
  parent_pattern?: string | null;
  /**
   * Creation timestamp
   */
  created_at: string;
  /**
   * Last update timestamp
   */
  updated_at?: string;
  /**
   * Closure timestamp
   */
  closed_at?: string | null;
  /**
   * Aggregated progress percentage
   */
  progress_percent?: number;
  /**
   * Total cost in USD
   */
  total_cost_usd?: number;
  /**
   * Total duration in seconds
   */
  duration_seconds?: number | null;
  schema_version: string;
  [k: string]: unknown;
}


// ===== pattern_member.json =====

/**
 * Membership of a stitch in a pattern
 */
export interface PatternMember {
  /**
   * Pattern ID
   */
  pattern_id: string;
  /**
   * Stitch ID
   */
  stitch_id: string;
  /**
   * When stitch was added
   */
  added_at?: string;
  /**
   * Who added this stitch
   */
  added_by?: string | null;
  schema_version: string;
  [k: string]: unknown;
}


// ===== pattern_query.json =====

/**
 * Saved query for auto-including stitches in a pattern
 */
export interface PatternQuery {
  /**
   * Pattern ID
   */
  pattern_id: string;
  /**
   * Query string for matching stitches
   */
  query: string;
  /**
   * When query was created
   */
  created_at?: string;
  schema_version: string;
  [k: string]: unknown;
}


// ===== presence.json =====

/**
 * Real-time multi-operator presence tracking (§19.4)
 */
export interface Presence {
  /**
   * Operator identifier (e.g., 'tailscale:user@example.com' or 'os:username')
   */
  operator_id: string;
  /**
   * Project name if viewing a project, null otherwise
   */
  project?: string | null;
  /**
   * Stitch ID if viewing a specific Stitch, null otherwise
   */
  stitch_id?: string | null;
  /**
   * Last heartbeat timestamp (records older than 30s are considered stale)
   */
  last_seen: string;
  /**
   * Privacy toggle: 'visible' shows presence to others, 'hidden' hides it
   */
  visibility: "visible" | "hidden";
  [k: string]: unknown;
}


// ===== pricing_config.json =====

/**
 * Provider pricing configuration for cost calculation
 */
export interface PricingConfig {
  /**
   * Pricing per adapter
   */
  adapters?: {
    [k: string]: {
      models?: {
        [k: string]: {
          /**
           * Input price per million tokens (USD)
           */
          input_per_million: number;
          /**
           * Output price per million tokens (USD)
           */
          output_per_million: number;
          /**
           * Cache read price per million tokens (USD)
           */
          cache_read_per_million?: number;
          /**
           * Cache write price per million tokens (USD)
           */
          cache_write_per_million?: number;
        };
      };
      /**
       * Default model if not specified
       */
      default_model?: string;
    };
  };
}


// ===== redaction_policy.json =====

/**
 * Per-project redaction policy override (§18.5). Controls how secrets are detected and handled in attachments, transcripts, and other content.
 */
export interface RedactionPolicy {
  /**
   * Action to take when secrets are detected: warn (log only), redact (replace with [REDACTED]), reject (block the operation)
   */
  action?: "warn" | "redact" | "reject";
  /**
   * Pattern sets to enable for this project. If omitted, all default patterns are enabled.
   */
  patterns?: (
    | "anthropic_api_key"
    | "generic_sk_key"
    | "aws_access_key"
    | "github_token"
    | "slack_token"
    | "jwt"
    | "bearer_token"
    | "env_var_secret"
    | "json_secret_field"
  )[];
  [k: string]: unknown;
}


// ===== reflection_ledger.json =====

/**
 * Learned rule or preference from operator behavior
 */
export interface ReflectionLedger {
  /**
   * Unique reflection identifier
   */
  id: string;
  /**
   * Scope: 'global', 'project:<name>', or 'pattern:<id>'
   */
  scope: string;
  /**
   * The learned rule
   */
  rule: string;
  /**
   * Why this rule was proposed
   */
  reason?: string;
  /**
   * Stitches that led to this rule
   */
  source_stitches?: string[];
  /**
   * Rule status
   */
  status: "proposed" | "approved" | "rejected" | "archived";
  /**
   * Creation timestamp
   */
  created_at: string;
  /**
   * Last time this rule was applied
   */
  last_applied?: string | null;
  /**
   * Number of times this rule was applied
   */
  applied_count?: number;
  /**
   * Who approved this rule
   */
  approved_by?: string | null;
  /**
   * Approval timestamp
   */
  approved_at?: string | null;
  /**
   * Archive timestamp
   */
  archived_at?: string | null;
  /**
   * SHA-256 hash of rule+reason for deduplication (§19.2)
   */
  content_hash?: string;
  /**
   * Number of times this proposal was rejected (prevents immediate re-proposal)
   */
  rejection_count?: number;
  schema_version: string;
  [k: string]: unknown;
}


// ===== script_manifest.json =====

/**
 * Manifest for operator-invoked and event-triggered scripts (§22.3). Scripts are single executable files in ~/.hoop/scripts/<name> with optional manifest.yml metadata.
 */
export interface ScriptManifest {
  /**
   * Script name (must match the executable filename)
   */
  name: string;
  /**
   * Human-readable description of what the script does
   */
  description?: string;
  /**
   * Visibility scope: 'global' scripts appear everywhere, 'project' scripts only on matching projects
   */
  scope?: "global" | "project";
  /**
   * List of project names where this script is available (only used when scope=project)
   */
  projects?: string[];
  /**
   * Execution timeout in seconds (default: 300 = 5 minutes)
   */
  timeout_secs?: number;
  /**
   * Optional argument schema for UI prompts
   */
  arguments?: {
    /**
     * Argument name
     */
    name: string;
    /**
     * What this argument does
     */
    description?: string;
    required?: boolean;
    /**
     * Default value if not provided
     */
    default?: {
      [k: string]: unknown;
    };
    [k: string]: unknown;
  }[];
  /**
   * Cron schedule for automatic execution (5-field format: min hour dom month dow). Example: '0 4 * * *' for daily at 4 AM.
   */
  schedule?: string;
  /**
   * How to handle overlapping executions: 'skip' (default) skips if previous run is still active, 'queue' waits for previous run to finish, 'parallel' allows concurrent runs
   */
  overlap_policy?: "skip" | "queue" | "parallel";
  /**
   * Event subscriptions - when matching events fire, this script runs with event JSON on stdin
   */
  on?: {
    /**
     * Event pattern to match (glob pattern, e.g., 'stitch.*', 'bead.closed')
     */
    event: string;
    /**
     * Project name filter (exact match or glob)
     */
    project?: string;
    /**
     * Kind filter (exact match)
     */
    kind?: string;
    /**
     * Adapter filter (exact match)
     */
    adapter?: string;
    /**
     * Result filter (success or failure)
     */
    result?: "success" | "failure";
    [k: string]: unknown;
  }[];
  [k: string]: unknown;
}


// ===== secret_pattern.json =====

/**
 * A named secret pattern for the secrets scanner (§18). Each pattern has a unique identifier, a human-readable name, a severity level, and one or more regular expressions that match secrets of this type.
 */
export interface SecretPattern {
  /**
   * Unique snake_case identifier for this pattern (e.g., 'anthropic_api_key'). Used as the canonical identifier across client and backend.
   */
  id: string;
  /**
   * Human-readable name for this pattern type (e.g., 'AWS Access Key').
   */
  name: string;
  /**
   * Severity level: high, medium, or low.
   */
  severity: "high" | "medium" | "low";
  /**
   * Regular expressions that match this secret type. Multiple patterns are OR'd together.
   */
  patterns: string[];
  [k: string]: unknown;
}


// ===== session_kind.json =====

/**
 * Classification of a CLI session based on its prefix tag
 */
export type SessionKind = Worker | "dictated" | "ad_hoc" | "operator";

export interface Worker {
  /**
   * Worker name (NATO identifier)
   */
  worker: string;
  /**
   * Bead ID being processed
   */
  bead: string;
  /**
   * Strand name (pluck, explore, mend, etc.)
   */
  strand?: string | null;
}


// ===== stitch_bead.json =====

/**
 * Link between a stitch and a bead
 */
export interface StitchBead {
  /**
   * Stitch ID
   */
  stitch_id: string;
  /**
   * Bead ID
   */
  bead_id: string;
  /**
   * Workspace path (raw, for display)
   */
  workspace: string;
  /**
   * Realpath-resolved workspace path (for joins and dedup)
   */
  canonical_workspace?: string;
  /**
   * Relationship type
   */
  relationship: "created-here" | "executing" | "referenced";
  /**
   * When the link was created
   */
  linked_at?: string;
  schema_version: string;
  [k: string]: unknown;
}


// ===== stitch_link.json =====

/**
 * Link between two stitches with cross-workspace tracking (§4.2)
 */
export interface StitchLink {
  /**
   * Source stitch ID
   */
  from_stitch: string;
  /**
   * Target stitch ID
   */
  to_stitch: string;
  /**
   * Link kind
   */
  kind: "spawned" | "references";
  /**
   * Workspace path of the source stitch (for cross-workspace dependency tracking)
   */
  workspace_from: string;
  /**
   * Workspace path of the target stitch (for cross-workspace dependency tracking)
   */
  workspace_to: string;
  /**
   * When the link was created
   */
  created_at?: string;
  schema_version: string;
  [k: string]: unknown;
}


// ===== stitch_message.json =====

/**
 * A single message in a stitch
 */
export interface StitchMessage {
  /**
   * Unique message identifier
   */
  id: string;
  /**
   * Parent stitch ID
   */
  stitch_id: string;
  /**
   * Message timestamp
   */
  ts: string;
  /**
   * Message role
   */
  role: "user" | "assistant" | "system" | "tool";
  /**
   * Message content
   */
  content: {
    [k: string]: unknown;
  };
  /**
   * Attachment paths
   */
  attachments?: string[];
  /**
   * Token count if applicable
   */
  tokens?: number | null;
  /**
   * Tool use details if role is tool
   */
  tool_use?: {
    [k: string]: unknown;
  } | null;
  schema_version: string;
  [k: string]: unknown;
}


// ===== stitch_preview.json =====

/**
 * Preview data for 'What Will This Take?' before submitting a Stitch
 */
export interface StitchPreview {
  schema_version: string;
  /**
   * Cost and duration prediction from similar historical Stitches
   */
  prediction?: {
    cost?: {
      /**
       * Median cost in USD
       */
      p50?: number;
      /**
       * 90th percentile cost in USD
       */
      p90?: number;
      /**
       * Number of data points
       */
      count?: number;
      [k: string]: unknown;
    };
    duration?: {
      /**
       * Median duration in seconds
       */
      p50?: number;
      /**
       * 90th percentile duration in seconds
       */
      p90?: number;
      /**
       * Number of data points
       */
      count?: number;
      [k: string]: unknown;
    };
    /**
     * Most likely adapter + model (e.g., 'claude:opus'). NOTE: based on adapter + model, NOT strand.
     */
    likely_adapter_model?: string | null;
    /**
     * Number of similar Stitches used
     */
    similar_count?: number;
    data_range?: {
      start?: string;
      end?: string;
      [k: string]: unknown;
    };
    [k: string]: unknown;
  } | null;
  /**
   * Matched risk patterns from the Fix Lineage library
   */
  risk_patterns?: {
    pattern?: {
      id?: string;
      name?: string;
      description?: string;
      fix_recommendation?: string;
      severity?: "low" | "medium" | "high" | "critical";
      category?: "performance" | "correctness" | "security" | "integration" | "code_quality" | "infrastructure";
      [k: string]: unknown;
    };
    confidence?: number;
    matched_keywords?: string[];
    matched_labels?: string[];
    [k: string]: unknown;
  }[];
  /**
   * Currently-claimed beads that may have file overlap conflicts
   */
  file_conflicts?: {
    bead_id?: string;
    title?: string;
    project?: string;
    overlapping_files?: string[];
    [k: string]: unknown;
  }[];
  /**
   * Similar historical Stitches for reference
   */
  similar_stitches?: {
    id?: string;
    title?: string;
    similarity?: number;
    [k: string]: unknown;
  }[];
  [k: string]: unknown;
}


// ===== streaming_content.json =====

/**
 * Streaming content update
 */
export interface StreamingContent {
  /**
   * Conversation ID
   */
  conversation_id: string;
  /**
   * Content fragment
   */
  content: string;
  /**
   * Unix timestamp in milliseconds
   */
  timestamp: number;
  [k: string]: unknown;
}


// ===== stuck_detector_config.json =====

/**
 * Stuck detector configuration for monitoring worker health (§C1). Detects workers that are idle, hung, or retrying the same bead repeatedly.
 */
export interface StuckDetectorConfig {
  /**
   * Idle timeout: no events, any type, for N seconds (default: 180 = 3 minutes)
   */
  idle_timeout_secs?: number;
  /**
   * Hard ceiling on total turn duration regardless of activity (default: 3600 = 1 hour)
   */
  max_runtime_secs?: number;
  /**
   * Extended idle tolerance once real content appeared (default: 600 = 10 minutes)
   */
  content_seen_grace_secs?: number;
  /**
   * No heartbeat state transition (Live<->Hung) for N seconds (default: 300 = 5 minutes)
   */
  heartbeat_transition_threshold_secs?: number;
  /**
   * Maximum retry attempts on the same bead before alerting (default: 3)
   */
  retry_threshold?: number;
}


// ===== template_field.json =====

/**
 * A field definition in a stitch template. Fields define user inputs that are substituted into the template body.
 */
export interface TemplateField {
  /**
   * Machine key used in {{key}} placeholders for substitution
   */
  key: string;
  /**
   * Human-readable label for the form input
   */
  label: string;
  /**
   * Whether the field is required
   */
  required: boolean;
  /**
   * Placeholder text for the input field
   */
  placeholder?: string | null;
  /**
   * Default value for the field
   */
  default?: string | null;
  [k: string]: unknown;
}


// ===== ui_config.json =====

/**
 * UI configuration
 */
export interface UiConfig {
  theme?: "light" | "dark" | "auto";
  default_project_sort?: "name" | "last_activity" | "worker_count";
  archive_after_days?: number;
}


// ===== ui_state.json =====

/**
 * UI state persisted across sessions
 */
export interface UiState {
  /**
   * Currently active project
   */
  active_project?: string | null;
  /**
   * Currently active stitch
   */
  active_stitch?: string | null;
  /**
   * Sidebar width in pixels
   */
  sidebar_width?: number;
  panel_layout?: {
    fleet_visible?: boolean;
    beads_visible?: boolean;
    conversations_visible?: boolean;
    patterns_visible?: boolean;
    [k: string]: unknown;
  };
  filters?: {
    bead_status?: ("open" | "closed")[];
    stitch_kind?: ("operator" | "dictated" | "worker" | "ad-hoc")[];
    /**
     * Persisted conversation-pane filter — fleet vs ad-hoc vs all
     */
    conversation_filter?: "all" | "fleet" | "operator" | "ad-hoc" | "dictated";
    show_archived?: boolean;
    [k: string]: unknown;
  };
  theme?: "light" | "dark" | "auto";
  schema_version: string;
  /**
   * Track which prompts have been dismissed (progressive capability introduction)
   */
  prompts_dismissed?: {
    /**
     * ISO 8601 timestamp when 'What's new' v1.1.0 prompt was dismissed, null if not dismissed
     */
    whats_new_v1_1_0?: string | null;
    /**
     * ISO 8601 timestamp when 'Reflection Ledger empty' prompt was dismissed
     */
    reflection_ledger_empty_30d?: string | null;
    /**
     * ISO 8601 timestamp when 'Pattern suggestion' prompt was dismissed
     */
    pattern_suggestion_10plus?: string | null;
    /**
     * ISO 8601 timestamp when 'Agent intro' prompt was dismissed
     */
    agent_intro?: string | null;
    /**
     * ISO 8601 timestamp when 'Mic intro' prompt was dismissed
     */
    mic_intro?: string | null;
    [k: string]: unknown;
  };
  /**
   * Track first usage of features for progressive introduction
   */
  feature_usage?: {
    /**
     * First timestamp when agent was used
     */
    agent_first_used?: string | null;
    /**
     * First timestamp when dictation/mic was used
     */
    mic_first_used?: string | null;
    /**
     * First timestamp when patterns were created/used
     */
    patterns_first_used?: string | null;
    /**
     * First timestamp when reflection ledger was used
     */
    reflection_ledger_first_used?: string | null;
    [k: string]: unknown;
  };
  /**
   * Last HOOP version seen by this operator (for 'What's new' detection)
   */
  last_seen_version?: string | null;
  /**
   * Global 'don't bug me' setting - when false, no progressive prompts are shown
   */
  prompts_enabled?: boolean;
  [k: string]: unknown;
}


// ===== unknown_event_samples_response.json =====

/**
 * Samples response for GET /api/diagnostics/unknown-events/samples
 */
export interface UnknownEventSamplesResponse {
  /**
   * Buffered unknown event samples (last N events)
   */
  samples: {
    /**
     * Adapter name (e.g., 'needle', 'heartbeats', 'claude')
     */
    adapter: string;
    /**
     * Unknown event kind/type
     */
    event_kind: string;
    /**
     * Raw event payload (truncated if too long)
     */
    raw_event: string;
    /**
     * ISO 8601 timestamp when the unknown event was recorded
     */
    timestamp: string;
    /**
     * Source file path (if applicable)
     */
    source_path?: string;
    /**
     * Line number in source file (if applicable)
     */
    line_number?: number;
  }[];
  /**
   * Total count of unknown events (hoop_unknown_event_total metric)
   */
  total_count: number;
  /**
   * HOOP daemon version (e.g., '0.1.0')
   */
  daemon_version: string;
  /**
   * Schema version (e.g., '1.0.0')
   */
  schema_version: string;
}


// ===== unknown_events_response.json =====

/**
 * Summary response for GET /api/diagnostics/unknown-events
 */
export interface UnknownEventsResponse {
  /**
   * Total count of unknown events (hoop_unknown_event_total metric)
   */
  total_count: number;
  /**
   * Breakdown of unknown events by adapter and event kind
   */
  labeled_totals: {
    /**
     * Adapter name (e.g., 'needle', 'heartbeats', 'claude')
     */
    adapter: string;
    /**
     * Unknown event kind/type
     */
    event_kind: string;
    /**
     * Count of unknown events for this adapter/kind pair
     */
    count: number;
  }[];
  /**
   * HOOP daemon version (e.g., '0.1.0')
   */
  daemon_version: string;
  /**
   * Schema version (e.g., '1.0.0')
   */
  schema_version: string;
}


// ===== voice_config.json =====

/**
 * Voice/dictation configuration
 */
export interface VoiceConfig {
  /**
   * Path to Whisper model
   */
  whisper_model_path?: string;
  /**
   * Hotkey binding
   */
  hotkey?: string;
  max_recording_seconds?: number;
}


// ===== worker_display_state.json =====

/**
 * Worker state for display (combines WorkerState with additional info)
 */
export interface WorkerDisplayState {
  state: "executing" | "idle" | "knot";
  /**
   * Bead ID (when executing)
   */
  bead?: string;
  /**
   * Adapter name (when executing)
   */
  adapter?: string;
  /**
   * Model name (when executing)
   */
  model?: string;
  /**
   * Last strand worked on (when idle)
   */
  last_strand?: string | null;
  /**
   * Reason for knot state (when knotted)
   */
  reason?: string;
  [k: string]: unknown;
}


// ===== worker_liveness.json =====

/**
 * Worker liveness state
 */
export type WorkerLiveness = "Live" | "Hung" | "Dead";


// ===== worker_metadata.json =====

/**
 * Worker metadata for worker sessions
 */
export interface WorkerMetadata {
  /**
   * Worker name
   */
  worker: string;
  /**
   * Bead ID
   */
  bead: string;
  /**
   * Strand name
   */
  strand?: string | null;
  [k: string]: unknown;
}


// ===== workspace_entry.json =====

/**
 * A workspace entry within a project
 */
export interface WorkspaceEntry {
  /**
   * Raw workspace path as provided by the operator (display-only)
   */
  path: string;
  /**
   * Realpath-resolved absolute path used for joins and dedup
   */
  canonical_path?: string;
  /**
   * Workspace role
   */
  role: "primary" | "manifests" | "source" | "secrets" | "docs";
  [k: string]: unknown;
}

