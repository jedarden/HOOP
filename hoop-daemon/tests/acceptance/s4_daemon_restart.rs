...
async fn s4_daemon_restart_no_bead_loss() {
    ...
    // Spawn first daemon
    let (base_url1, _daemon1) = spawn_test_daemon_with_config::<fn(&mut Config)>(Some(|config| {
    ...
    let (base_url2, _daemon2) = spawn_test_daemon_with_config::<fn(&mut Config)>(Some(|config| {
        config.observer_mode = false;
    }))
    .await
...
async fn s4_daemon_quick_rebuild() {
    ...
    // Spawn first daemon
    let (base_url1, _daemon1) = spawn_test_daemon()
        .await
        .expect("Failed to spawn first daemon");
    ...
    // First daemon resources are cleaned up when dropped

    // Spawn second daemon and time the rebuild
    let rebuild_start = std::time::Instant::now();

    let (base_url2, _daemon2) = spawn_test_daemon()
        .await
        .expect("Failed to spawn second daemon");
...
async fn s4_fleet_unaffected_by_restart() {
    ...
    // First daemon resources cleaned up (simulating restart)
...
async fn s4_state_consistency_across_restarts() {
    ...
    for cycle in 0..3 {
        ...
        // Daemon resources cleaned up when dropped
