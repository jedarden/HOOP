...
async fn ws_and_rest_return_consistent_state() {
    ...
    assert_eq!(
        ...
        rest_beads.len(),
        "WS and REST bead counts should match"
    );

    // Verify worker counts match
    assert_eq!(
        ws_snapshots.worker_count,
        rest_workers.len(),
        "WS and REST worker counts should match"
    );
    ...
    assert_eq!(
        ...
        rest_projects.len(),
        "WS and REST project counts should match"
    );
...
async fn rest_api_endpoints_return_valid_state() {
    ...
    let testrepo_found = projects
        .iter()
        .any(|p| p.get("name").and_then(|n| n.as_str()) == Some("testrepo"));
...
async fn ws_reconnect_rebuilds_state() {
    ...
    if let Message::Text(text) = snapshots_msg {
        let event: serde_json::Value = serde_json::from_str(&text).unwrap();
...
async fn test_state_projections_contain_required_fields() {
    ...
    for worker in &workers {
        ...
        assert!(
            worker.get("state").is_some(),
            "Each worker must have a 'state' field"
        );
    }
