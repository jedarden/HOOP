...
fn test_substantial_stdout_generation() {
    ...
    assert!(
        verify_size_requirement(total_bytes, 10_240),
        "Generated stdout must be at least 10KB, got {} bytes",
        total_bytes
    );
    ...
    println!(
        "=== Substantial stdout generation test completed - Generated {} bytes in {} lines ===",
...
fn test_reusable_large_stdout_generation() {
    ...
    assert!(
        verify_size_requirement(bytes1, 10_240),
        "Default config must generate >10KB"
    );
    ...
    assert!(
        verify_size_requirement(bytes2, 20_000),
        "Large config must generate >20KB"
    );
    assert!(
        bytes2 > bytes1,
        "Large config should generate more output than default"
    );
    ...
    assert_eq!(
        bytes2, bytes3,
        "Same configuration should produce identical output size"
    );

    eprintln!("VERIFICATION_METADATA: Reusable function test completed successfully");
    eprintln!(
        "VERIFICATION_METADATA: Test 1 (default): {} bytes in {} lines",
        bytes1, lines1
    );
    eprintln!(
        "VERIFICATION_METADATA: Test 2 (large): {} bytes in {} lines",
        bytes2, lines2
    );
    eprintln!(
        "VERIFICATION_METADATA: Test 3 (determinism): {} bytes == {} bytes",
        bytes2, bytes3
    );
