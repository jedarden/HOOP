...
async fn concurrent_switch_requests_are_handled_gracefully() {
    ...
    // Issue concurrent switch requests - clone client for both tasks
    let client_clone1 = FailoverClient {
        base_url: _base_url.clone(),
        client: client.client.clone(),
    };
    let client_clone2 = FailoverClient {
        base_url: _base_url.clone(),
        client: client.client.clone(),
    };
...
/// Handler that returns 503 Service Unavailable for all Anthropic requests
async fn mock_anthropic_handler() -> impl axum::response::IntoResponse {
    use axum::Json;
    (
...
impl MockAnthropicServer {
    ...
    async fn new() -> anyhow::Result<Self> {
        ...
        let app = Router::new().route("/v1/messages", post(mock_anthropic_handler));
