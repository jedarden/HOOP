...
#[tokio::test]
#[serial]
#[ignore = "Phase 5 test - temporarily disabled for Phase 1 CI gate (bf-5mpcl)"]
...
async fn test_handoff_context_includes_reflection_ledger() {
    ...
    conn.execute(
        r#"INSERT INTO reflection_ledger (id, scope, rule, reason, status, created_at, content_hash)
           VALUES (?1, 'global', 'test rule 1', 'testing', 'approved', ?2, ?3)"#,
        [uuid::Uuid::new_v4().to_string(), now.clone(), format!("hash1_{}", uuid::Uuid::new_v4())],
    )
    .unwrap();

    conn.execute(
        r#"INSERT INTO reflection_ledger (id, scope, rule, reason, status, created_at, content_hash)
           VALUES (?1, 'project:test', 'test rule 2', 'testing', 'approved', ?2, ?3)"#,
        [uuid::Uuid::new_v4().to_string(), now.clone(), format!("hash2_{}", uuid::Uuid::new_v4())],
    )
    .unwrap();
    ...
    conn.execute(
        r#"INSERT INTO reflection_ledger (id, scope, rule, reason, status, created_at, content_hash)
           VALUES (?1, 'global', 'rejected rule', 'testing', 'rejected', ?2, ?3)"#,
        [uuid::Uuid::new_v4().to_string(), now.clone(), format!("hash3_{}", uuid::Uuid::new_v4())],
    )
    .unwrap();
