...
3. Log rotation — **DONE** (`hoop-ttb.7.5`): `hoop-daemon/src/log_rotation.rs` rotates at 100MB/24h and prunes files whose date is older than `MAX_AGE_DAYS` (14) on every startup and rotation. **Revisit the retention window itself**, though: observed 2026-07-22, `~/.hoop/logs/` had grown to 26G across the daemon's first 13 days of uptime (started 2026-07-09) — the 14-day cutoff hadn't fired yet (nothing was old enough to prune), but extrapolating the growth rate, a full 14-day retention window steady-states around ~28G, roughly 28x over this phase's own "<1GB in logs + backups" success criterion below. Either shorten `MAX_AGE_DAYS` (7 days ≈ 14G, still over target) or reduce write volume — the dominant cost is `hoop_daemon::beads::Quarantined malformed bead line` WARN spam recurring across many workspaces. **Confirmed 2026-07-26 (`bf-4hu5k`, see `notes/bf-4hu5k.md`): this is a real parse bug, not noise — schema drift between HOOP's `Bead` struct and the actual `br` JSONL format quarantines 100% of bead lines in affected workspaces (e.g. `bead-forge`, `mobile-gaming`), emitting 694,005 WARNs (~91 MB, ~100% of one day's log) and leaving those workspaces' bead views silently empty. Four independent mismatches each suffice: `created_by`/`dependencies` missing-and-required, plus `BeadStatus`/`BeadType` enums expecting PascalCase but `br` writes lowercase (`closed`/`task`/`blocked`/…) with values that have no variant. Fix the struct + de-dup the WARN; that single change drops volume to the INFO baseline and is the primary lever for the `<1GB/month` criterion — shrinking `MAX_AGE_DAYS` alone cannot reach it. (Implementation deferred to a dedicated code bead under the Phase 1 gate.) Note this is unrelated to NEEDLE's own `~/.needle/logs/`, which independently reached 63G the same day from stale `echo-test`/canary test artifacts and abandoned `archive/.tmp-*` snapshots — a NEEDLE-side gap, out of HOOP's scope, but worth a heads-up to whoever owns that repo.
4. `/healthz` + `/readyz`
