...
fn scan_with_no_interactive_flag_auto_registers() {
    ...
    assert!(
        ...
        "Test workspace should have .beads/"
    );
...
fn remove_with_no_interactive_requires_confirm_flag() {
    ...
    let code = std::fs::read_to_string("src/projects.rs").expect("Failed to read projects.rs");
...
fn test_remove_flag_confirms_with_confirm_flag() {
    ...
    assert!(
        ...
        "Should have confirm requirement check"
    );
    ...
    assert!(
        ...
        "Should have prompt suppression check"
    );
...
fn test_remove_short_flag_variant() {
    ...
    assert!(
        result.is_ok(),
        "Should successfully parse short flag variant"
    );
    let parsed = result.unwrap();
...
fn restore_with_no_interactive_requires_confirm_flag() {
    ...
    let code = std::fs::read_to_string("src/restore.rs").expect("Failed to read restore.rs");
...
fn test_restore_parse_flag_before_subcommand() {
    ...
    let result =
        parse_flag_before_subcommand(&["restore", "--from", "s3://my-bucket/backups/snap-001"]);
    ...
    assert_eq!(
        ...
        "Should identify 'restore' as command"
    );
    assert!(
        parsed.args.contains(&"restore".to_string()),
        "Should include 'restore' in args"
    );
    assert!(
        parsed.args.contains(&"--from".to_string()),
        "Should include --from flag"
    );
    assert!(
        parsed
            .args
            .contains(&"s3://my-bucket/backups/snap-001".to_string()),
        "Should include URI"
    );
}
...
fn test_restore_parse_flag_after_subcommand() {
    ...
    let result =
        parse_flag_after_subcommand(&["restore", "--from", "s3://my-bucket/backups/snap-001"]);
...
fn no_interactive_flag_propagates_from_main_to_handlers() {
    ...
    let code = std::fs::read_to_string("src/main.rs").expect("Failed to read main.rs");
...
fn verify_safe_operation_pattern_for_scan() {
    ...
    let _scan_end = code.find("\npub fn ").unwrap_or(code.len());
    // Find the next function boundary
    let next_fn_start = code[scan_start + 1..]
        .find("\npub fn ")
        .map(|i| scan_start + 1 + i);
...
fn verify_destructive_operation_pattern_for_remove_and_restore() {
    ...
    let projects_code =
        std::fs::read_to_string("src/projects.rs").expect("Failed to read projects.rs");
    let restore_code =
        std::fs::read_to_string("src/restore.rs").expect("Failed to read restore.rs");
