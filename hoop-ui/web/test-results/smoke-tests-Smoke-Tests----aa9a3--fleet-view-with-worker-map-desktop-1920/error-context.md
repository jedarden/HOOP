...
- Name: smoke-tests.spec.ts >> Smoke Tests - Fleet View >> should load fleet view with worker map
- Location: e2e/smoke-tests.spec.ts:251:3
...
Locator: locator('.fleet-map, .worker-grid')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.fleet-map, .worker-grid')
...
```ts
  155 | 
  156 |   test('should display Fleet Map tab content', async ({ page }) => {
  157 |     const firstCard = page.locator('.project-card-fleet').first();
  158 | 
  159 |     const cardCount = await firstCard.count();
  160 |     if (cardCount === 0) {
  161 |       test.skip(true, 'No projects available');
  162 |       return;
  163 |     }
  164 | 
  165 |     await firstCard.click();
  166 |     await page.waitForSelector('.app-project-detail', { timeout: 5000 });
  167 | 
  168 |     // Click Fleet Map tab
  169 |     const fleetTab = page.locator('button[role="tab"]', { hasText: 'Fleet Map' });
  170 |     const tabCount = await fleetTab.count();
  171 |     if (tabCount > 0) {
